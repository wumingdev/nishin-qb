                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                -- TriggerServerEvent('police:server:policeAlert', Lang:t("info.robnpc") )
                                                -- Npcattackrandom = math.random(1, 100)
                                                -- if Npcattackrandom > 50 then
                                                --     local playerPed = ped
                                                --     -- 取消逃跑行為
                                                ...
                                                --     -- 創建一個名為 "ROB_TARGET" 的新群組
                                                --     AddRelationshipGroup("ROB_TARGET")

                                                --     -- 獲取 "ROB_TARGET" 群組的哈希值
                                                --     local npcGroupHash = GetHashKey("ROB_TARGET")

                                                --     -- 將 NPC 分配給 "ROB_TARGET" 群組
                                                --     SetPedRelationshipGroupHash(targetPed, npcGroupHash)

                                                --     -- 設置 "ROB_TARGET" 群組與玩家所在群組之間的關係為敵對
                                                --     SetRelationshipBetweenGroups(5, npcGroupHash, GetPlayerGroup(playerPed))
                                                --     return
                                                -- end
