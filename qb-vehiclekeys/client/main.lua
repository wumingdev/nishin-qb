...
-- 解鎖車輛
RegisterNetEvent('vehiclekeys:client:UnlockClosestVehicle', function()
    local playerPed = PlayerPedId()
    local coords = GetEntityCoords(playerPed)
    local vehicle = QBCore.Functions.GetClosestVehicle(coords)
    local plate = QBCore.Functions.GetPlate(vehicle)
    
    QBCore.Functions.Progressbar("hotwire_vehicle", "使用萬用鑰匙中", 10000, false, true, {
        ...
        QBCore.Functions.Notify('拿到了鑰匙', "success")
        KeysList[plate] = true
    end, function() -- Cancel
        StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
    end)
    ...
    if DoesEntityExist(vehicle) then
        ...
        if lockStatus ~= 1 then
            ...
            QBCore.Functions.Notify("車輛解鎖成功", "success")
        else
            QBCore.Functions.Notify("車輛並未上鎖", "primary")
        end
    else
        QBCore.Functions.Notify("目前附近沒有車輛", "error")
    end
end)
