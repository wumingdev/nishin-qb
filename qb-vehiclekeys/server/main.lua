...
CreateThread(function()
    local vehicles = MySQL.query.await('SELECT `citizenid`, `plate`, `mods` FROM `player_vehicles`', {})
    local mismatchs = {}
    for k,v in pairs(vehicles) do
        if not VehicleList[v.plate] then VehicleList[v.plate] = {} end
        VehicleList[v.plate][v.citizenid] = true
        local mods = json.decode(v.mods)

        if v.plate ~= mods.plate then
            table.insert(mismatchs, {v.plate, mods.plate})
            mods.plate = v.plate
            
            MySQL.Async.execute('UPDATE `player_vehicles` SET `mods` = @mods WHERE `citizenid` = @citizenid AND `plate` = @plate', {
    ...
    -- log json about wrong plates
    print("VehicleKeys: Found "..#mismatchs.." vehicles with mismatching plates.")
    print("VehicleKeys: Mismatching plates:")
    for _,v in ipairs(mismatchs) do
        print(json.encode(v))
    end
...
function GiveKeys(id, plate)
    ...
    print("Gave keys to "..GetPlayerName(id).." for "..plate)
