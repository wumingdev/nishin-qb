...
RegisterNetEvent('qb-bossmenu:server:GradeUpdate', function(data)
	...
	if Employee then
		if Employee.Functions.SetJob(Player.PlayerData.job.name, data.grade) then
			...
			TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "你已經被升職成" ..data.gradename..".", "success")
		else
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			...
			if Employee.Functions.SetJob("unemployed", '0') then
				...
				TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , "你被開除了，滾", "error")
			else
...
RegisterNetEvent('qb-bossmenu:server:HireEmployee', function(recruit)
	...
	if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
		...
		TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , "你已被雇用成" .. Player.PlayerData.job.label .. "", "success")
