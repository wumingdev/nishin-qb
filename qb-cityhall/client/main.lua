                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['qb-core']:DrawText('[E] 參加駕駛課程')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['qb-core']:DrawText('[E] 開啟市民服務')
                            end
                        else
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['qb-core']:DrawText('[E] 開啟市政廳') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestLicenses', function(_, cb)
    ...
    availableLicenses = Config.Cityhalls[closestCityhall].licenses
...
-- 領代幣
RegisterNUICallback('qb-cityhall:client:claimItem', function()
    -- TriggerEvent('chat:addMessage', {args = {json.encode({'觸發領錢'})}})
    TriggerServerEvent('qb-cityhall:server:claimItem')
    -- cb('ok')
end)