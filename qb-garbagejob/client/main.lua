        ...
        if shouldContinue then
            if not garbageVehicle then
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 0.5) then
                        QBCore.Functions.SpawnVehicle(Config.Vehicle, function(veh)
                            SetVehicleEngineOn(veh, false, true)
                            garbageVehicle = veh
                            SetVehicleNumberPlateText(veh, "TRASH-" .. tostring(math.random(100, 999)))
                        ...
                        end, vector3(v.x, v.y, v.z), true, false)
                        return
                    else
                        occupied = true
                    end
