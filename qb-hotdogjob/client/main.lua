    ...
    QBCore.Functions.TriggerCallback('qb-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            if Config.UseTarget then
                ...
                ReqControlFromEntity(StandObject)
...
RegisterNetEvent('qb-hotdogjob:staff:DeletStand', function()
    ...
    local Object = GetClosestObjectOfType(pos.x, pos.y, pos.z, 10.0, `prop_hotdogstand_01`, false, false, false)
    print(pos)
    print(Object)
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            print("stand be a entityas network")
            Wait(100)
            NetworkRequestControlOfEntity(Object)
            print("stand be control")
            if not IsEntityAMissionEntity(Object) then
                print("Is not a mission entity")
                SetEntityAsMissionEntity(Object, true, true)
                print("Is a mission entity")
            end
            Wait(100)
            DeleteEntity(Object)
            QBCore.Functions.Notify(Lang:t("info.admin_removed"), 'success')
        end
