...
local function RequestEntityContorl(entityId)
    ...
    print('recv contorl of entity', entity, NetworkHasControlOfEntity(entity))
end
...
RegisterNetEvent('qb-taxi:client:DoTaxiNpc', function()
    ...
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            RequestCollisionAtCoord(Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].x, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].y, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].z)

            NpcData.Npc = CreatePed(26, model, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].x, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].y, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].z - 0.98, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].w, false, false)
            TaskSetBlockingOfNonTemporaryEvents(NpcData.Npc, true)
            PlaceObjectOnGroundProperly(NpcData.Npc)
            FreezeEntityPosition(NpcData.Npc, true)
            CreateThread(function()
                while not HasCollisionLoadedAroundEntity(NpcData.Npc) do
                    RequestCollisionAtCoord(Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].x, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].y, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].z)
                    Wait(100)
                end
                        ...
                        if dist < 25 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    -- FreezeEntityPosition(NpcData.Npc, false)
                                    RequestEntityContorl(NpcData.Npc)
                                    TaskEnterVehicle(NpcData.Npc, veh, -1, freeSeat, 1.0, 0)
                                    SetTimeout(3000, function()
                                        RequestEntityContorl(NpcData.Npc)
                                        TaskWarpPedIntoVehicle(NpcData.Npc, veh, freeSeat)
                                    end)
                                    resetMeter()
    ...
    CreateThread(function()
        while not NpcData.NpcTaken do
            ...
            if isInsidePickupZone then
                if IsControlJustPressed(0, 38) then
                    local ped = PlayerPedId()  -- 獲取當前玩家的Ped實體
                    if IsPedInAnyVehicle(ped, false) then  -- 檢查玩家是否在車內
                        local vehicle = GetVehiclePedIsIn(ped, false)  -- 獲取玩家所在的車輛
                        FreezeEntityPosition(vehicle, true)  -- 凍結車輛位置
                        Citizen.Wait(6000)  -- 等待6000毫秒，即5秒
                        FreezeEntityPosition(vehicle, false)  -- 解除凍結
                    end
