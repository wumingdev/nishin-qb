...
AddEventHandler('playerDropped', function()
    local src = source
    if dutyPlayers and next(dutyPlayers) then
        for k, v in pairs(dutyPlayers) do
            if v.source == src then
                if v.job == "police" then
                    currentCops = currentCops - 1
                end
                table.remove(dutyPlayers, k)
                break
            end
        end
    end
...
AddEventHandler('QBCore:Server:PlayerLoaded', function(player)
    ...
    if not hasFound then
        if (player.PlayerData.job.name == "police" or player.PlayerData.job.name == "ambulance") and player.PlayerData.job.onduty then
            ...
            local heading = GetEntityHeading(GetPlayerPed(targetSource))
            dutyPlayers[#dutyPlayers+1] = {
                source = targetSource,
...
local function UpdateBlips()
    ...
    TriggerLatentClientEvent('police:client:UpdateBlips', -1, 25000, dutyPlayers)
...
QBCore.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("police:client:ImpoundVehicle", src, false, 200)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)
...
RegisterNetEvent('police:server:deleteObject', function(objectId)
    TriggerClientEvent('police:client:removeObject', objectId)
end)
