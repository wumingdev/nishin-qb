...
local currentSpawnIndex = 1 -- 定義當前重生點索引
function TakeOutVehicle(vehicleInfo)
    -- print(vehicleInfo)
    local spawnPoints = Config.Locations["SpawnVehicle"]
    local coords = spawnPoints[currentSpawnIndex] -- 獲取當前重生點的坐標
    local maxAttempts = #spawnPoints -- 設置最大嘗試次數
    local attempts = 0 -- 計數當前的嘗試次數

    -- 檢查指定的重生點是否被車輛佔用
    while IsPositionOccupied(coords.x , coords.y , coords.z , 0, false, true, false, false, false, 0, true) 
        and attempts < maxAttempts do

        currentSpawnIndex = currentSpawnIndex + 1 -- 如果被佔用，則增加索引
        attempts = attempts + 1 -- 增加嘗試次數

        if currentSpawnIndex > #spawnPoints then
            currentSpawnIndex = 1 -- 如果超出最大索引，則重置為1
        end

        coords = spawnPoints[currentSpawnIndex] -- 獲取新的重生點坐標
    end
...
function MenuGarage(currentSelection)
    ...
    if IsArmoryWhitelist() then
        for category, vehicles in pairs(authorizedVehicles) do
            vehicleMenu[#vehicleMenu+1] = {
                header = vehicles.name, -- 將類別作為 header
                txt = vehicles.txt, -- 車輛標籤作為內容文字
                params = {
                    event = "police_car_Submenu",
...
--組合台子菜單
RegisterNetEvent('police_car_Submenu', function(args)
    local currentSelection = args.currentSelection

    local submenu = {
        { header = "選擇成品", txt = "選擇要製造的成品", isMenuHeader = true },
    }

    for categoriesName, categories in pairs(args.vehicle) do
...
-- 返回組裝主菜單
RegisterNetEvent('MenuGarage', MenuGarage)
...
if Config.UseTarget then
    ...
    for _, v in pairs(Config.Locations["armory"]) do
        armouryZones[#armouryZones+1] = BoxZone:Create(
            vector3(vector3(v.x, v.y, v.z)), 5, 2.5, {
            name="box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1,
